...
interface CreateAppointmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (appointmentData: Omit<Appointment, 'id' | 'dashboardId' | 'createdAt' | 'updatedAt'>) => Promise<void>;
}

export const CreateAppointmentModal = ({ isOpen, onClose, onSubmit }: CreateAppointmentModalProps) => {
  ...
  const onFormSubmit = async (data: AppointmentFormData) => {
    ...
    if (isExpense && data.paymentMethod) {
      (appointmentData as Omit<ExpenseAppointment, 'id' | 'dashboardId' | 'createdAt' | 'updatedAt'>).paymentMethod = data.paymentMethod;
    }

    await onSubmit(appointmentData as Omit<Appointment, 'id' | 'dashboardId' | 'createdAt' | 'updatedAt'>);
    reset();
    onClose();
  };
